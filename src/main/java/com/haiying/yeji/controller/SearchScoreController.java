...
public class SearchScoreController {
    ...
    @GetMapping("list2")
    public IPage<ScoreResult22> list2(int current, int pageSize, Integer year) {
        ...
        wrapper.eq(ScoreResult22::getYear, year).eq(ScoreResult22::getDepttName, checkUser.getDeptName());

        return scoreResult22Service.page(new Page<>(current, pageSize), wrapper);
    }

    @GetMapping("list")
    public ResponseResult list(int current, int pageSize, Integer year) {
        if (year.equals(2022)) {
            CheckUser checkUser = (CheckUser) httpSession.getAttribute("user");
            ...
            IPage<ScoreResult22> list = scoreResult22Service.page(new Page<>(current, pageSize), wrapper);
